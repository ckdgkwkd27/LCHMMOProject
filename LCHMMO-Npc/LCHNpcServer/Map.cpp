...
bool Map::CanGo(Vector2Int cellPos, bool checkActor)
{
	...
	std::ignore = checkActor;

	int32 x = cellPos.x - MinX;
	int32 y = MaxY - cellPos.y;
	bool ret = !CollisionBuf[y][x];
	return ret;
}
...
bool Map::ApplyMove(ActorPtr actor, Vector2Int dest)
{
	...
	protocol::PositionInfo posInfo = actor->ActorInfo.objects().posinfo();
	{
	...
	Vector2Int cellPos(actor->ActorInfo.objects().posinfo().posx(), actor->ActorInfo.objects().posinfo().posy());
	actor->ActorInfo.mutable_objects()->mutable_posinfo()->set_posx(dest.x);
	actor->ActorInfo.mutable_objects()->mutable_posinfo()->set_posy(dest.y);
	return true;
}
